...
public class A_Two_Substrings {
    ...
    public static void s() {
        ...
        boolean[][] right = new boolean[s.length()][2];
        for(int i=s.length()-2; i>=0; i--) {
            right[i][0]|=right[i+1][0];
            right[i][1]|=right[i+1][1];
            if(s.subSequence(i, i+2).equals("AB")) right[i][0] = true;
            else if(s.subSequence(i, i+2).equals("BA")) right[i][1] = true;
        }
        for(int i=0; i<s.length()-3; i++) {
            if(right[i+2][1] && s.subSequence(i, i+2).equals("AB")) {
                p.writeln("YES");
                return;
            }
            else if(right[i+2][0] && s.subSequence(i, i+2).equals("BA")) {
                p.writeln("YES");
                return;
            }
        }
