...
public class Ashim_2 {
    public static void s() {
        String a = sc.nextLine();
        String b = sc.nextLine();
        long count = 0;
        HashMap<Character, Long> amap = new HashMap<>(), bmap = new HashMap<>();
        for(char c : a.toCharArray()) {
            amap.put(c, amap.getOrDefault(c, 0l) + 1);
        }
        for(char c : b.toCharArray()) {
            bmap.put(c, bmap.getOrDefault(c, 0l) + 1);
        }
        for(Map.Entry<Character, Long> mp : amap.entrySet()) {
            count += mp.getValue()*bmap.getOrDefault(mp.getKey(), 0l);
        }
        p.writeln(count);
    }
