...
public class B_Berland_Crossword {
    ...
    public static boolean check(int n, int u, int r, int l, int d) {
        return u >= 0 && r >= 0 && l >= 0 && d >= 0 && u<=n-2 && r <= n-2 && l <= n-2 && d <= n-2;
    } 
    public static void s() {
        int n = sc.nextInt(), u = sc.nextInt(), r = sc.nextInt(), d = sc.nextInt(), l = sc.nextInt();
        for(int i=0; i<=15; i++) {
            int tl = ((1<<0)&i) !=0?1:0;
            int tr = ((1<<1)&i) !=0?1:0;
            int bl = ((1<<2)&i) !=0?1:0;
            int br = ((1<<3)&i) !=0?1:0;
            if(check(n, u-tl-tr, r-tr-br, l-tl-bl, d-bl-br)) {
                p.writeln("YES");
                return;
            }
        }
