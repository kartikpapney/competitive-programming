...
public class B_Deadly_Laser {
    public static boolean check(int n, int m, int sx, int sy, int d) {
        for(int j=1; j<=m; j++ ) {
            if(Math.abs(sx-1)+Math.abs(sy-j) <= d) return false; 
        }
        for(int i=1; i<=n; i++) {
            if(Math.abs(sx-i)+Math.abs(sy-m) <= d) return false;
        }
        return true;
    }
    public static boolean check2(int n, int m, int sx, int sy, int d) {
        for(int i=1; i<=n; i++) {
            if(Math.abs(sx-i)+Math.abs(sy-1) <= d) return false;
        }
        for(int j=1; j<=m; j++ ) {
            if(Math.abs(sx-n)+Math.abs(sy-j) <= d) return false; 
        }
        return true;
    }
    public static void s() {
        ...
        if(check(n, m, sx, sy, d) || check2(n, m, sx, sy, d)) {
            p.writeln(ans);
        } else {
            p.writeln(-1);
        }
