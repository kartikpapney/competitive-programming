...
public class B_GCD_Compression {
    public static void s() {
        ...
        for(int i=0; i<arr.length; i++) {
            int val = arr[i];
            if((val&1) == 0) even.add(new int[]{val, i+1});
            else odd.add(new int[]{val, i+1});
        }
        ...
        for(int i=0; i<p; i+=2) pr.writeln(odd.get(i)[1] + " " + odd.get(i+1)[1]);
        for(int i=0; i<q; i+=2) pr.writeln(even.get(i)[1] + " " + even.get(i+1)[1]);
    }
