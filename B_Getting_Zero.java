...
public class B_Getting_Zero {
    ...
    static Integer[][] dp = new Integer[32769][20];
    public static int find(int val, int opn) {
        if(val == 0) return 0;
        if(opn == 18) return 100;
        if(dp[val][opn] != null) return dp[val][opn];
        return dp[val][opn] = Math.min(find((2*val)%32768, opn+1), find((val+1)%32768, opn+1)) + 1;
    }
