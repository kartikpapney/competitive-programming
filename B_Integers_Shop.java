...
public class B_Integers_Shop {
    public static void s() {
        ...
        HashMap<Integer, Integer> minm = new HashMap<>();
        HashMap<Integer, Integer> maxm = new HashMap<>();
        ...
        for(int i=0; i<n; i++) {
            ...
            arrr.add(arr[i][0]);
            arrr.add(arr[i][1]);
            map.put(arrr, Functions.min(map.getOrDefault(arrr, Integer.MAX_VALUE), arr[i][2]));


            minm.put(arr[i][0], Functions.min(minm.getOrDefault(arr[i][0], Integer.MAX_VALUE), arr[i][2]));
            maxm.put(arr[i][1], Functions.min(maxm.getOrDefault(arr[i][1], Integer.MAX_VALUE), arr[i][2]));
            
            if(arr[i][0] < arr[min][0]) min = i;
            if(arr[i][1] > arr[max][1]) max = i;
            
            ArrayList<Integer> x = new ArrayList<>();
            x.add(arr[min][0]);
            x.add(arr[max][1]);
            p.writeln(Functions.min(map.getOrDefault(x, Integer.MAX_VALUE), minm.get(arr[min][0]) + maxm.get(arr[max][1])));
        }
    }
