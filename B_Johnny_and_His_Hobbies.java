...
public class B_Johnny_and_His_Hobbies {
    public static boolean find(int[] arr, int n, int asf) {
        if(n == -1) {
            if(asf == 0) return false;
            HashSet<Integer> set = new HashSet<>();
            for(int val : arr) set.add(val^asf);
            for(int val : arr) if(!set.contains(val)) return false;
            p.writeln(asf);
            return true;
        }
        boolean a = find(arr, n-1, asf);
        if(a) return a;
        boolean b = find(arr, n-1, asf|(1<<n));
        return b;
