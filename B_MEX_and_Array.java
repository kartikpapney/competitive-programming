...
public class B_MEX_and_Array {
    ...
    static String solve(int a, int b, int c) {
        // code here
        Queue<Character> p = new ArrayDeque<>();
        Pair[] arr = new Pair[3];
        arr[0] = new Pair('a', a);
        arr[1] = new Pair('b', b);
        arr[2] = new Pair('c', c);
        Arrays.sort(arr, (x, y) -> (y.count - x.count));
        ...
        arr[2].count -= arr[2].count;
        for(int i=0; i<arr[1].count; i++) {
            p.add((char)arr[0].c);
            p.add((char)arr[1].c);
        }
        arr[0].count -= arr[1].count;
        arr[1].count -= arr[1].count;
        StringBuilder strb = new StringBuilder();
        while(arr[0].count != 0 && !p.isEmpty()) {
            if(arr[0].c != p.peek()) {
                strb.append("" + (char)arr[0].c);
                arr[0].count--;
            }
            strb.append("" + (char)p.poll());
        }
        while(!p.isEmpty()) {
            strb.append((char)p.poll());
        }
