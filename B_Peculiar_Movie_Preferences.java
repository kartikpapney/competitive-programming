...
public class B_Peculiar_Movie_Preferences {
    public static boolean check() {
        int n = sc.nextInt();
        String[] arr = new String[n];
        HashSet<String> two = new HashSet<>(), three = new HashSet<>();
        for(int i=0; i<n; i++) {
            arr[i] = sc.nextLine();
        }
        for(String s : arr) {
            ...
            if(s.length() == 2) {
                two.add(s);
                if(two.contains(rev.toString())) return true;
                for(char c='a'; c <= 'z'; c++) {
                    if(three.contains(rev.toString()+c)) return true;
                }
            } else {
                three.add(s);
                if(three.contains(rev.toString())) return true;
                if(two.contains(rev.toString().substring(0, 2))) return true;
            }
        }
        return false;
    }
