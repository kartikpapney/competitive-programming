...
public class B_Quality_vs_Quantity {
    ...
    public static void s() {
        ...
        PriorityQueue<Long> ap = new PriorityQueue<>(), bp = new PriorityQueue<>(Collections.reverseOrder());
        long a = 0, b = 0;
        Functions.sort(arr);
        for(long val : arr) {
            ap.add(val); bp.add(val);
        }
        a = ap.poll();
        int c  = 1;
        while(a >= b && !ap.isEmpty() && !bp.isEmpty() && c < arr.length) {
            a += ap.poll();
            b += bp.poll();
            c += 2;
        }
