...
public class B_RPG_Protagonist {
    public static boolean debug = false;
    static void debug(String st) {
        if(debug) p.writeln(st);
    }
    public static void s() {
        long mycap = sc.nextLong(), fwercap = sc.nextLong();
        long maxsword = sc.nextLong(), maxaxes = sc.nextLong();
        long wsword = sc.nextLong(), waxes = sc.nextLong();
        if(wsword > waxes) {
            maxsword = maxaxes^maxsword;
            maxaxes = maxaxes^maxsword;
            maxsword = maxsword^maxaxes;
        ...
        long ans = 0;
        for(long currsword = 0; currsword<=maxsword; currsword++) {
            if(currsword*wsword > mycap) break;
            long s1 = currsword;
            long a1 = Math.min((mycap - s1*wsword)/waxes, maxaxes);

            long s2 = Math.min(fwercap/wsword, maxsword - currsword);
            long a2 = Math.min((fwercap - s2*wsword)/waxes, (maxaxes - a1));
            ans=Math.max(ans, s1 + a1 + s2 + a2);
        }
        p.writeln(ans);
    }
