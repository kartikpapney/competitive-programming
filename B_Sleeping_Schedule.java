...
public class B_Sleeping_Schedule {
    public static boolean debug = false;
    static void debug(String st) {
        if(debug) p.writeln(st);
    }
    public static void s() {
        int n = sc.nextInt(), h = sc.nextInt(), l = sc.nextInt(), r = sc.nextInt();
        int[] arr = sc.readArray(n);
        int[][] dp = new int[n+2][h];
        for(int i=n; i>=1; i--) {
            for(int j=0; j<h; j++) {
                int ti = (arr[i-1] + j)%h;
                int tj = (arr[i-1] - 1 + j)%h;
                int ii = (ti >= l && ti <= r)?1:0;
                int ij = (tj >= l && tj <= r)?1:0;
                dp[i][j] = Math.max(dp[i+1][ti] + ii, dp[i+1][tj] + ij);
            }
        }
        p.writeln(dp[1][0]);
    }
