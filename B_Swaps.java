...
public class B_Swaps {
    ...
    public static void s() {
        int n = sc.nextInt();
        int[] odd = sc.readArray(n);
        int[] even = sc.readArray(n);

        HashMap<Integer, Integer> emap = new HashMap<>(), omap = new HashMap<>();
        ...
        for(int i=1; i<=2*n; i+=2) {
            ans = Math.min(ans, omap.get(i) + emap.get(i+1));
        }
        p.writeln(ans);
    }
