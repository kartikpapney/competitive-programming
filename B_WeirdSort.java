...
public class B_WeirdSort {
    public static boolean find(ArrayList<Integer>[] graph, int src, int des, HashSet<Integer> visited) {
        if(src == des) return true;
        visited.add(src);
        for(int val : graph[src]) {
            if(!visited.contains(val)) {
                boolean check = find(graph, val, des, visited);
    ...
    public static void s() {
        ...
        int[] pos = sc.readArray(m);

        HashMap<Integer, Integer> map = new HashMap<>(), sort = new HashMap<>();
        for(int i=0; i<arr.length; i++) map.put(arr[i], i+1);
        Functions.sort(arr);
        for(int i=0; i<arr.length; i++) sort.put(arr[i], i+1);
        ArrayList[] graph = new ArrayList[n+1];
        for(int i=0; i<graph.length; i++) graph[i] = new ArrayList<>();
        for(int i=0; i<pos.length; i++) {
            graph[pos[i]].add(pos[i] + 1);
            graph[pos[i] + 1].add(pos[i]);
        }
        for(int val : arr) {
            int p1 = map.get(val);
            int p2 = sort.get(val);
             if(!find(graph, p1, p2, new HashSet<>())) {
                 p.writeln("NO");
                 return;
             }
