...
public class Bitwise_swaps {
    public static boolean debug = false;
    static void debug(String st) {
        if(debug) p.writeln(st);
    }
    static void findConnectedComponent(ArrayList<ArrayList<Integer>> graph, int bit, boolean[] visited, HashSet<Integer> comp) {
        if(visited[bit]) return;
        visited[bit] = true;
        for(int val : graph.get(bit)) {
            comp.add(val);
            for(int i=0; i<32; i++) {
                if(((1<<i)&val) != 0) findConnectedComponent(graph, i, visited, comp);
            }
        }
    }
    public static void s() {
        ...
        for(int i=0; i<32; i++) graph.add(new ArrayList<>());
        for(int i=0; i<arr.length; i++) {
            for(int j=0; j<32; j++) {
                if(((1<<j)&arr[i]) != 0) graph.get(j).add(arr[i]);
            }
        }
        ...
        for(int i=0; i<32; i++) {
            ...
            if(!visited[i]) findConnectedComponent(graph, i, visited, set);
            map.put(i, set);
            if(!set.isEmpty()) {
        ...
        for(int i=0; i<arr.length; i++) {
            if(arr[i] != srr[i]) {
                ...
                for(int k=0; k<32; k++) {
                    if(((1<<k)&arr[i]) != 0) {
                        if(map.get(k).contains(srr[i])) {
                            check = true;
                            break;
                        }
                    }
                    if(((1<<k)&srr[i]) != 0) {
                        if(map.get(k).contains(arr[i])) {
                            check = true;
                            break;
                        }
                    }
