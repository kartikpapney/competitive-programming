...
public class C_Adding_Powers {
    ...
    public static void s() {
        ...
        long copy = 1;
        while(copy <= Math.pow(10, 16)) {
            brr.add(copy);
            copy*=k;
        }
        PriorityQueue<Long> pq = new PriorityQueue<>(Collections.reverseOrder());
        for(long v : arr) if(v != 0) pq.add(v);
        if(pq.isEmpty()) {
            p.writeln("YES");
            return;
        }
        int idx = brr.size()-1;
        while(idx >= 0 && pq.peek() < brr.get(idx)) idx--;
        while(!pq.isEmpty() && idx >= 0) {
            // System.out.println(pq);
            long poll = pq.poll();
            if(poll - brr.get(idx) > 0) {
                pq.add(poll - brr.get(idx));
            } else if(poll - brr.get(idx) < 0) {
                pq.add(poll);
            }
            idx--;
        }
