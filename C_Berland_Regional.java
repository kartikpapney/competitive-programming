...
public class C_Berland_Regional {
    public static void s() {
        int n = sc.nextInt();
        long[] u = sc.readLongArray(n);
        long[] s = sc.readLongArray(n);
        HashMap<Long, ArrayList<Long>> map = new HashMap<>();
        for(int i=0; i<n; i++) {
            map.putIfAbsent(u[i], new ArrayList<>());
            map.get(u[i]).add(s[i]);
        }
        for(Map.Entry<Long, ArrayList<Long>> mp : map.entrySet()) {
            Collections.sort(mp.getValue(), Collections.reverseOrder());
        }
        int size = 0;
        for(Map.Entry<Long, ArrayList<Long>> mp : map.entrySet()) {
            ArrayList<Long> arr = mp.getValue();
            for(int i=1; i<arr.size(); i++) arr.set(i, arr.get(i-1) + arr.get(i));
        }
        ArrayList<Long> as = new ArrayList<>();
        for(int i=0; i<n; i++) as.add(0l);
        for(Map.Entry<Long, ArrayList<Long>> mp : map.entrySet()) {
            size = Math.max(size, mp.getValue().size());
        }
        for(int k=1; k<=n; k++) {
            long ans = 0;
            for(Map.Entry<Long, ArrayList<Long>> mp : map.entrySet()) {
                ...
                ans += arr.get((arr.size()/k)*k - 1);
            }
            as.set(k-1, ans);
        }
        for(long val : as) p.writes(val);
        p.writeln();
    }
