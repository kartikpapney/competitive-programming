...
public class C_Digital_Logarithm {
    ...
    static int check(int a, int b) {
        if(a == b) return 0;
        if(opn(a) == b || a == opn(b)) return 1;
        if(opn(a) == opn(b) || opn(opn(a)) == b || a == opn(opn(b))) return 2;
        if(opn(opn(a)) == opn(b) || opn(a) == opn(opn(b)) || opn(opn(opn(a))) == b || a == opn(opn(opn(b)))) return 3;
        return 4;
    }
    public static void s() {
        ...
        HashMap<Integer, Integer> bb = new HashMap<>(), aa = new HashMap<>();
        int opn = 0;
        for(int val : b) {
            if(map.containsKey(val)) {
                ...
                if(val <= 9) {
                    bb.put(val, bb.getOrDefault(val, 0) + 1);
                } else {
                    opn++;
                    bb.put(opn(val), bb.getOrDefault(opn(val), 0) + 1);
                }
            }
        }
        for(Map.Entry<Integer, Integer> mp : map.entrySet()) {
            if(mp.getKey() <= 9) {
                for(int k=0; k<mp.getValue(); k++) {
                    aa.put(mp.getKey(), aa.getOrDefault(mp.getKey(), 0) + 1);
                }
            } else {
                int val = opn(mp.getKey());
                opn+=mp.getValue();
                for(int k=0; k<mp.getValue(); k++) {
        ...
        int one = 0;
        for(int i=1; i<=9; i++) {
            int value = Math.abs(aa.getOrDefault(i, 0) - bb.getOrDefault(i, 0));
            rem.put(i, value);
            if(i > 1) remaining+=value;
            else one += value;
        }
        opn+=one;
        remaining-=one;
