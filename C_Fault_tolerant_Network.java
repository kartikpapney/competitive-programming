...
public class C_Fault_tolerant_Network {
    ...
    public static long two(long[] arr, long[] brr, int n) {
        long a = Math.abs(arr[0] - brr[0]) + Math.abs(arr[n-1] - brr[n-1]);
        long b = Math.abs(arr[0] - brr[n-1]) + Math.abs(arr[n-1] - brr[0]);
        return Math.min(a, b);
    }
    public static long three(long[] arr, long[] brr, int n) {
        long a = Integer.MAX_VALUE, b = Integer.MAX_VALUE, c = Integer.MAX_VALUE, d = Integer.MAX_VALUE;
        for(int i=0; i<arr.length; i++) {
            a = Math.min(a, Math.abs(arr[0] - brr[i]));
            b = Math.min(b, Math.abs(brr[0] - arr[i]));
            c = Math.min(c, Math.abs(arr[n-1] - brr[i]));
            d = Math.min(d, Math.abs(brr[n-1] - arr[i]));
        }
        return Math.min(
                Math.min(Math.abs(arr[0] - brr[0]) + c + d
                ,Math.abs(arr[n-1] - brr[n-1]) + a + b),
                Math.min(Math.abs(arr[0] - brr[n-1]) + c + b
                ,Math.abs(arr[n-1] - brr[0]) + a + d));
    }
    public static long four(long[] arr, long[] brr, int n) {
        long a = Integer.MAX_VALUE, b = Integer.MAX_VALUE, c = Integer.MAX_VALUE, d = Integer.MAX_VALUE;
        for(int i=0; i<arr.length; i++) {
            a = Math.min(a, Math.abs(arr[0] - brr[i]));
            b = Math.min(b, Math.abs(brr[0] - arr[i]));
    ...
    public static void s() {
        ...
        p.writeln(Functions.min(two(a, b, n), three(a, b, n), four(a, b, n)));
    }
