...
public class C_Hamburgers {
    ...
    static boolean canMake(long[] n, long[] p, long b, long s, long c, long mid, long r) {
        r -= Math.max(0l, (mid*b - n[0])*p[0]);
        r -= Math.max(0l, (mid*s - n[1])*p[1]);
        r -= Math.max(0l, (mid*c - n[2])*p[2]);
        return r >= 0l;
    }
    public static void s() { 
        String st = sc.nextLine();
        long[] n = sc.readLongArray(3), p = sc.readLongArray(3);
        long r = sc.nextLong();
        long b = 0, s = 0, c = 0;
        ...
        while(low <= high) {
            ...
            if(debug) pr.writeln(mid);
            if(canMake(n, p, b, s, c, mid, r)) {
                low = mid+1;
                ans = mid;
            } else {
                high = mid-1;
    ...
    static final Print pr = new Print();
