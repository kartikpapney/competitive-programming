...
public class C_Jury_Meeting {
    public static void s() {
        ...
        int max = Functions.max(arr);
        int count = Functions.count(arr, max);
        if(count >= 2) {
            p.writeln(Functions.factorial(n));
        } else if((count = Functions.count(arr, max-1))!= 0){
            long ans = 1;
            while(n != 1) {
                if(n != (count+1)) ans = Functions.mod_mul(ans, n);
                n--;
            }
            p.writeln(Functions.mod_mul(ans, count));
        } else {
            p.writeln(0);
        }
    }
