...
public class C_Kill_the_Monster {
    public static void s() {
        double hc = sc.nextLong(), dc = sc.nextLong(), hm = sc.nextLong(), dm = sc.nextLong();
        double k = sc.nextLong(), w = sc.nextLong(), a = sc.nextLong();
        for(int x=0; x<=k; x++) {
            double first = (hc + x*a)/dm + 1;
            double second = Math.ceil(hm/(dc + (k - x)*w));
            if(first > second) {
                // pr.writeln( ((hc + x*a)/dm + 1) + " " + (hm/(dc + (k - x)*w)));
                pr.writeln("YES");
                return;
            }
        }
