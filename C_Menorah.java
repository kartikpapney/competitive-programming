    ...
    public static void s() {
        ...
        for(int i=0; i<n; i++) {
            if(a.charAt(i) == b.charAt(i)) {
                pf++;
                if(a.charAt(i) == '1') hasp = true;
            } else {
                npf++;
                if(a.charAt(i) == '1') hasn = true;
            }
        }
        int ans = Integer.MAX_VALUE;
        if(npf == 0) ans = 0;
        else if(hasp && npf == n-1) ans = 1;
        if(hasn) {
            if(npf%2 == 0) ans = Math.min(ans, npf);
        }
        if(hasp) {
            if(pf%2 != 0) ans = Math.min(ans, pf);
        }
        p.writeln(((ans == Integer.MAX_VALUE)?-1:ans));
    }
