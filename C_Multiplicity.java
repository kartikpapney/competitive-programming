...
public class C_Multiplicity {
    ...
    public static void s() {
        ...
        for(int j=0; j<arr.length; j++) {
            ArrayList<Integer> factor = factor(arr[j]);
            for(int i=factor.size()-1; i>=0; i--) {
                int val = factor.get(i);
                dp[val] = Functions.mod_add(dp[val], dp[val-1]);
            }
        }
        long ans = 0;
        for(int i=1; i<dp.length; i++) ans = Functions.mod_add(ans, dp[i]);
        p.writeln(ans);
    }
