...
public class C_Palindrome_Basis {
    ...
    public static void s() {
        ...
        //         int times = -1;
        //         if(j%arr.get(i-1) == 0) {
        //             times = j-((j/arr.get(i-1)) + 1)*arr.get(i-1);
        //         } else {
        //             times = j-((j/arr.get(i-1)))*arr.get(i-1);
        //         }
        //         // if(times < 0) dp[j]+=pref[j];
        //         // else dp[j] += pref[j] - pref[times];
        //         for(int k=j; k>times && k >= 0; k--) dp[j]+=prev[k];
        //     }
        //     p.writes(dp);
        //     p.writeln();
        // }
        ...
        for(int i=1; i<=arr.size(); i++) {
            ...
            for(int j=0; j<dp.length; j++) {
                int times = 1;
                while(j-times*arr.get(i-1) >= 0) {
                    dp[j] = (dp[j] + prev[j-times++*arr.get(i-1)])%(1000_000_007);
                }
            }
        }
        // p.writeln(dp[tar]);
