...
public class C_Permute_Digits {
    ...
    public static void s() {
        ...
        long cpyb = b;
        if(count(a) < count(b)) {
            long[] arr = new long[count(a)];
            for(int i=arr.length-1; i>=0; i--) {
                arr[i] = a%10;
                a/=10;
            }
            Functions.reverseSort(arr);
            long ans = 0;
            ...
            for(int i=0; i<brr.length; i++) {
                PriorityQueue<Long> extra = new PriorityQueue<>(Collections.reverseOrder());
                while(check && !pq.isEmpty() && pq.peek() > brr[i]) {
                    extra.add(pq.poll());
                }
                if(pq.isEmpty()) {
                    long mod = ans%10;
                    pq.add(mod);
                    ans/=10;
                    while(extra.peek() == mod) {
                        pq.add(extra.poll());
                    }
                    ans = ans*10 + extra.poll();
                    while(!extra.isEmpty()) pq.add(extra.poll());
                    ans = ans*10+pq.poll();
                    check = false;
                } else {
                    if(pq.peek() < brr[i]) check = false;
                    ans = ans*10+pq.poll();
                    while(!extra.isEmpty()) pq.add(extra.poll());
                }
            }
