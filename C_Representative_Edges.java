...
public class C_Representative_Edges {
    public static void s() {
        ...
        for(int val : arr) {
            ...
            len = Math.max(len, map.get(val));
        }
        // case 2: form the arithmetic sequence
        for(int start = 0; start < arr.length-1; start++) {
            for(int end = start + 1; end < arr.length; end++) {
                ...
                for(int i=start + 1; i<=end; i++) {
                    if(((arr[end] - arr[start])*(i-start))%(x-1) == 0) {
                        int extra = ((arr[end] - arr[start])*(i-start))/(x-1);
                        if(extra == arr[i] - arr[start]) {
                            cnt++;
                        }
                    }
                }
