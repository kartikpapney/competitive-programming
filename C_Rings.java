...
// done
public class C_Rings {
    public static void s() {
        int n = sc.nextInt();
        String s = sc.nextLine();
        int zeroLeft = -1, zeroRight = -1;
        for(int i=0; i<s.length()/2; i++){ if(s.charAt(i) == '0') {zeroLeft = i + 1; break;} };
        for(int i=s.length()/2; i<s.length(); i++) if(s.charAt(i) == '0') {zeroRight = i + 1; break;};
        if(zeroLeft == -1 && zeroRight == -1) {
            p.writeln(1 + " " + (s.length()-1) + " " + 2 + " " + s.length());
        } else if(zeroLeft == -1) {
            p.writeln(1 + " " + zeroRight + " " + 1 + " " + (zeroRight - 1));
        } else {
            p.writeln(zeroLeft + " " + n + " " + (zeroLeft + 1) + " " + n);
        }
    }
