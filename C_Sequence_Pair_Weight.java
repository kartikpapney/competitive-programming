...
public class C_Sequence_Pair_Weight {
    ...
    public int longestPalindrome(String[] words) {
        ...
        for(Map.Entry<String, Integer> mp : map.entrySet()) {
            ...
            Integer kvalue = mp.getValue();
            StringBuilder rev = key.reverse();
            Integer rvalue = map.getOrDefault(rev, 0);
            if(rev.equals(key)) {
                ans += kvalue/2;
            } else {
                ans += Math.min(kvalue, rvalue);
            }
        }
        return ans*2;
    }
    ...
    public static void s() {
        ...
        for(Map.Entry<Integer, ArrayList<Integer>> mp : map.entrySet()) {
            count += (mp.getValue().size()*(mp.getValue().size()-1));
        }
        p.writeln(count);
    }
