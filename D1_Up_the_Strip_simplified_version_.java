...
public class D1_Up_the_Strip_simplified_version_ {
    public static void s() {
        ...
        for(long x=2; x<= n; x++) {
            ...
            f[(int)x] = (val + f[(int)x-1])%MOD;
            val = (val + f[(int)x-1])%MOD;
            // for(long z = 2; z*z < x; z++) {
            //     f[(int)x] = (f[(int)x] + f[(int)(x/z)])%MOD;
            // }
            for(long c = 1; c*c <= x; c++) {
                long len = (x/c) - (x/(c+1));
                f[(int)x] = (f[(int)x] + (f[(int)c]*len)%MOD)%MOD;
            }
        }
        for(long v : f) p.writes(v);
        p.writeln();
        p.writeln(f[n] + "");
    }
