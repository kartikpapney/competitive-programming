...
public class D2_Magic_Powder_2 {
    public static boolean debug = false;
    static boolean can(long[] quantity, long[] has, long mid, long k) {
        for(int i=0; i<quantity.length; i++) {
            k -= Math.max(0, (quantity[i]*mid - has[i]));
            if(k < 0) return false;
        }
        if(debug) p.writeln(k);
        return true;
    }
    ...
    public static void s() {
        int n = sc.nextInt();
        long k = sc.nextLong();
        long[] quantity = sc.readLongArray(n);
        long[] has = sc.readLongArray(n);
        long low = 0;
        long high = 0;
        for(int i=0; i<n; i++) high = Math.max(high, (k + has[i])/quantity[i]);
        long ans = 0;
        while(low <= high) {
            long mid = low + (high - low)/2;
            if(can(quantity, has, mid, k)) {
                low = mid+1;
                ans = mid;
            } else {
                high = mid-1;
