...
public class D_1_Xor_Subsequence_easy_version {
    private static int find(List<Integer> arr, int val) {
        int start = 0;
        int end = arr.size()-1;
        int idx = arr.size();
        while(start <= end) {
    ...
    public static int lengthOfLIS(int[] nums) {
        ...
        for(int val : nums){
            if(val == 0) {
                ...
                nrr.add(Integer.MIN_VALUE);
                nrr.addAll(arr);
                arr = nrr;
                continue;
            }
            int idx = find(arr, val);
            if(idx == arr.size()) {
                arr.add(val);
            } else {
                arr.set(idx, val);
