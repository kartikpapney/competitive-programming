...
public class D_Decrease_the_Sum_of_Digits {
    ...
    public static void s() {
        ...
        for(int i=0; i<sum.length; i++) {
            if(sum[i] > s) {
                int pos = i-1;
                while(sum[pos] + 1 > s) pos--;
                StringBuilder strb = new StringBuilder();
                strb.append("1"); 
                strb.append("0".repeat(n.substring(pos).length()));
                StringBuilder strb2 = new StringBuilder();
                strb2.append(n.substring(pos));
                BigInteger a = new BigInteger(strb.toString());
                BigInteger b = new BigInteger(strb2.toString());
                BigInteger res = a.subtract(b);
                p.writeln(res.toString());
                return;
            }
        }
        p.writeln(0);
