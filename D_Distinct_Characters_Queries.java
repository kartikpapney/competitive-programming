...
public class D_Distinct_Characters_Queries {
    public static void s() {
        ...
        for(char c='a'; c<='z'; c++) map.put(c, new TreeSet<>());
        ...
        while(qq-- != 0) {
            ...
            if(q == 1) {
                ...
                char pchar = index.get(pos);
                index.put(pos, c);
                map.get(pchar).remove(pos);
                map.get(c).add(pos);
            } else {
                int l = sc.nextInt();
                int r = sc.nextInt();
                int ans = 0;
                for(char c='a'; c<='z'; c++) {
                    TreeSet<Integer> position = map.get(c);
                    if(position.ceiling(l) != null && position.ceiling(l)<= r) ans++;
                }
                p.writeln(ans);
            }
        }
