...
public class D_Infinite_Set {
    ...
    static boolean reduce(long n, HashSet<Long> set) {
        while(n > 1) {
            if(n%2 == 1) {
                n>>=1;
            } else if(n%4 == 0) {
    ...
    public static void s() {
        ...
        fib[0] = 1;
        fib[1] = 1;
        for(int i=2; i<fib.length; i++) fib[i] = (fib[i-1] + fib[i-2])%MOD;
        for(int i=1; i<fib.length; i++) fib[i] = (fib[i] + fib[i-1])%MOD;
        for(long val : arr) {
            if(reduce(val, set)) {
                set.add(val);
                String binary = Long.toBinaryString(val);
                if(binary.length() <= pp) {
                    ans = (ans + fib[(int)(pp - binary.length())])%MOD;
                }
            }
        }
        p.writeln(ans);
