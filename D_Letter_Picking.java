...
public class D_Letter_Picking {
    ...
    public static boolean check(String s, int i, int j, char c, int turn) {
        String idx = i + " " + j + " " + c + " " + turn;
        if(i > j) {
            return false;
        }
        boolean ans = false;
        ...
        if(turn == 0) {
            // alice ki bari hai
            if(s.charAt(i) <= s.charAt(j)) ans |= check(s, i+1, j, s.charAt(i), (turn+1)%2);
            if(s.charAt(i) >= s.charAt(j))  ans |= check(s, i, j-1, s.charAt(j), (turn+1)%2);
        } else {
            // bob
            char ch = s.charAt(i);
            if(ch > s.charAt(j)) ch = s.charAt(j);
            draw &= (ch == c);
            if(s.charAt(i) <= c) ans |= check(s, i+1, j, s.charAt(i), (turn+1)%2);
            if(s.charAt(j) <= c) ans |= check(s, i, j-1, s.charAt(j), (turn+1)%2);
        }
        dp.put(idx, !ans);
        return !ans;
    }
    public static void s() {
        ...
        //     }
        // }
        boolean ans = check(s, 0, s.length()-1, 'a', 0);
        if(!draw) {
            if(!ans) p.writeln("Alice");
            else p.writeln("Bob");
        } else {
            p.writeln("Draw");
        }
