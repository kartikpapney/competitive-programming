...
public class D_Longest_k_Good_Segment {
    ...
    public static void s() {
        ...
        for(int i=0; i<arr.length; i++) {
            ...
            len++;
            while(map.size() > k) {
                map.put(arr[i-len+1], map.get(arr[i-len+1]) - 1);
                if(map.get(arr[i-len+1]) == 0) map.remove(arr[i-len+1]);
                len--;
            }
            if(maxlen < len) {
                maxlen = len;
                result = new int[]{i-len+2, i+1};
            }
        }
        p.writeln(result[0] +  " " + result[1]);
    }
