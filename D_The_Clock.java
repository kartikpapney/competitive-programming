...
public class D_The_Clock {
    ...
    public static void s() {
        ...
        for(int i=0; ; i++) {
            int increament = hour*60 + minute + i*x;
            int nhour = (increament/60)%24;
            int nminute = increament%60;
            String sss = (nhour<=9?"0":"") + nhour + (nminute<=9?"0":"") + nminute;
            if(set.contains(sss))break;
            if(isPalindrome(sss)) cnt++;
            set.add(sss);
        }
        p.writeln(cnt);
    }
