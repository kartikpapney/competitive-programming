...
public class D_The_Number_of_Imposters {
    static boolean check = true;
    static int[] cnt = new int[2];
    public static void colorGraph(ArrayList<ArrayList<Integer>> graph, int src, int[] color, int clr, int n) {
        if(src <= n) cnt[color[src]]++;
        for(int val : graph.get(src)) {
            if(color[val] == -1) {
                color[val] = (clr^1);
                colorGraph(graph, val, color, (clr^1), n);
            } else if(color[val] != (clr^1)) {
                check = false;
                return;
            }
        }
    ...
    public static void s() {
        ...
        ArrayList<ArrayList<Integer>> graph = new ArrayList<>(n+k+1);
        for(int i=0; i<n+k+1; i++) graph.add(new ArrayList<>());
        for(int i=0; i<graph.size(); i++) graph.set(i, new ArrayList<>());
        ...
        for(int m=0; m<k; m++) {
            ...
            if(c == 'i') {
                ...
                graph.get(i).add(fake);
                graph.get(fake).add(i);
                graph.get(fake).add(j);
                graph.get(j).add(fake);
                fake++;
            }
        }
        // for(int i=0; i<graph.size(); i++) {
        //     p.writes(i + " -> ");
        //     for(int val : graph.get(i)) p.writes(val);
        //     p.writeln();
        // }
        int ans = 0;
        for(int i=1; i<=n; i++) {
