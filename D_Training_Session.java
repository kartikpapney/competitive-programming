...
public class D_Training_Session {
    public static long countRAT(int[][] arr) {
        HashMap<Integer, Integer> l = new HashMap<>();
        HashMap<Integer, Integer> r = new HashMap<>();
        for(int[] val : arr) {
            l.put(val[0], l.getOrDefault(val[0], 0) + 1);
            r.put(val[1], r.getOrDefault(val[1], 0) + 1);
        }
        long ans = 0;
        for(int[] val : arr) {
            ans += (1l*l.get(val[0]) - 1)*(1l*r.get(val[1]) - 1);
        }
        return ans;
    }
    public static void s() {
        ...
        long ans = (1l*n*(n-1)*(n - 2))/6 - countRAT(arr);
        p.writeln(ans);
    }
