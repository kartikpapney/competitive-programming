...
public class E_1_Erase_and_Extend_Easy_Version {
    public static String get(StringBuilder strb, int  k) {
        StringBuilder ans = new StringBuilder();
        while(k >= strb.length()) {
            ans.append(strb.toString());
            k-=strb.length();
        }
        ans.append(strb.substring(0, k));
        return ans.toString();
    }
    public static void s() {
        ...
        String ans = get(new StringBuilder(s), k);
        StringBuilder strb = new StringBuilder();
        for(int i=0; i<n; i++) {
            strb.append(s.charAt(i));
            String cans = get(strb, k);
            if(ans.compareTo(cans) > 0) ans = cans;
        }
        p.writeln(ans.toString());
    }
