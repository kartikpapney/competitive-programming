...
public class J_Jeopardy_of_Dropped_Balls {
    public static int check(int[][] arr, int i, int j, boolean[] check) {
        if(i == arr.length) return j+1;
        if(arr[i][j] != 2) {
            check[0] = false;
            if(arr[i][j] == 1) {
                arr[i][j] = 2;
                return check(arr, i, j + 1, check);
            } else {
                arr[i][j] = 2;
                return check(arr, i, j - 1, check);
            }
        }
        return check(arr, i+1, j, check);
    }
    public static void s() {
        ...
        for (int i = 0; i < c.length; i++) {
            if(map.containsKey(c[i])) {
                p.writes(map.get(c[i]));
            } else {
                boolean[] check = new boolean[]{true};
                int ans = check(arr, 0, c[i]-1, check);
                if(check[0]) {
                    map.put(c[i], ans);
                }
                p.writes(ans);
