...
class Encrypter {
    ...
    public Encrypter(char[] keys, String[] values, String[] dictionary) {
        ...
        for(int i=0; i<keys.length; i++) {
            ...
            map.putIfAbsent(values[i], new HashSet<>());
    ...
    public int decrypt(String word2) {
        ...
        for(String s : dict) {
            boolean check = true;
            if(2*(s.length()) != word2.length()) {
                continue;
            }
            int idx = 0;
            for(char c : s.toCharArray()) {
                if(!map.getOrDefault(s.substring(idx, idx+2), new HashSet<>()).contains(c)) {
                    check = false;
                    break;
                }
                idx+=2;
...
public class Main {
    ...
    public List<List<Integer>> findWinners(int[][] matches) {
        ...
        for(int[]  mat : matches) {
            if(!map.containsKey(mat[0])) a.add(mat[0]);
            else if(map.get(mat[0]) == 1) b.add(mat[0]);
            
            if(!map.containsKey(mat[1])) a.add(mat[1]);
            else if(map.get(mat[1]) == 1) b.add(mat[1]);
        }
        List<Integer> aans = new ArrayList<>(), bans = new ArrayList<>();
        for(int val : a) aans.add(val);
        for(int val : b) bans.add(val);
